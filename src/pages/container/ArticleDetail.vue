<template>
  <div v-viewCount="{articleDetailId:this.articleDetail.id}">
    <div class=""  style="background: #fff;text-align: left">
...
<!--            </span>-->
      <header style="text-align: center">
        ...
        <h2>
          <i class="fa fa-fw fa-user"></i>发表于 <span >{{this.detailObj.createTime}}</span> •
          <i class="fa fa-fw fa-eye"></i>{{this.detailObj.viewCount}} 次围观 •
        </h2>
        <div class="ui label">
          <a :href="'#/Share?classId='+this.detailObj.categoryId">{{this.detailObj.categoryName}}</a>
        </div>
      </header>
      <div class="article-content markdown-body"  v-html="this.detailObj.content"></div>
    </div>
    <Comment></Comment>
  </div>
</template>
...
export default {
  ...
  methods:{
    ...
    async getArticleDetail(){
      ...
      await this.$store.dispatch('getArticleDetail',articleId);
      this.detailObj = this.articleDetail;
      const markdownIt = mavonEditor.getMarkdownIt()
      // markdownIt.re
      this.detailObj.content = markdownIt.render(this.articleDetail.content);
    },
    //获取评论列表
    async getCommentList(){
      let articleId=parseInt(this.$route.query.articleId);
      const {pageNum,pageSize} = this;
      await this.$store.dispatch('getCommentList',{articleId,pageNum,pageSize});
    },
  },
  components:{
    Comment
  ...
  computed:{
    ...mapState({
      articleDetail:state=>state.home.articleDetail
    })
  },
  watch:{
    $route:{
