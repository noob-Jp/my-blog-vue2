      ...
      <div v-show="isEdit" class="tcommonBox">
        ...
        <section>
          <ul class="userInfoBox">
            <li class="avatarlist">
              ...
              <el-upload
                ...
                <img   v-if="userInfoObj.avatar" :src="userInfoObj.avatar?userInfoObj.avatar:''"  :onerror="$store.state.errorImg" class="avatar">
          ...
          <div style="float: right">
            ...
            <el-button @click="saveInfoFun">保存</el-button>
          </div>
        </section>
      </div>
      <div v-show="!isEdit" class="tcommonBox">
...
import {reqGetUserInfo, reqUpdataUserInfo, reqUpdateUserInfo} from '@/api/index'
...
export default {
  ...
  methods: { //事件处理器
    handleAvatarSuccess(res, file) {//上传头像
      if(res.code === 200){
        this.userInfoObj.avatar = `http://${res.data}`;
    ...
    async saveInfoFun(){//保存编辑的用户信息
      ...
      let result=await reqUpdateUserInfo(this.userInfoObj)
      console.log(result);
  ...
  created() { //生命周期函数
    ...
    this.uploadURL = "http://localhost:7777/upload"
  }
}
</script>
